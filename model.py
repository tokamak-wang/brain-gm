...
class ModuleBARO(nn.Module):
    ...
    def forward(self, x, node_axis=1):
        ...
        num_time,batch,nodes,feature = x.shape
        x_mean = x.mean(node_axis)
        x = x.permute(1, 2, 3, 0).reshape(batch, nodes, feature* num_time)
...
class ModuleMamba(nn.Module):
    def __init__(self,input_dim, hidden_dim):
        super().__init__()
        self.config = MambaConfig(d_model=hidden_dim, n_layers=1)
...
class ModelBRAINGM(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_classes,num_heads, num_layers, num_time, sparsity, dropout=0.5, cls_token='sum', readout='sero', garo_upscale=1.0):
        super().__init__()
        ...
        if readout=='baro': readout_module = ModuleBARO
        ...
        for i in range(num_layers):
            ...
            self.mamba_modules.append(ModuleMamba(hidden_dim, 2*hidden_dim))
    ...
    def forward(self, v, a, t, sampling_endpoints):
        ...
        for layer, (G, R, M, L) in enumerate(zip(self.gnn_layers, self.readout_modules, self.mamba_modules, self.linear_layers)):
            h = G(h, a)
        ...
        attention['node-attention'] = torch.stack(attention['node-attention'], dim=0).detach().cpu()
